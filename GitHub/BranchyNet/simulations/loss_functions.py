...
def loss(loss_func, exit_tag, device, pred, target, conf, cost):
    ...
    * cost:         cost rate of the each exit point.
    ...
    if loss_func == 'v5':
        return loss_v5(loss_func, pred, device, target, conf, cost)


def loss_v5(loss_func, pred, device, target, conf, cost):
    criterion = torch.nn.CrossEntropyLoss()

    # 最終出力も含めるため、num_ee + 1 個の要素を持つ配列を作る (1, 2, 3, 4, 5)
